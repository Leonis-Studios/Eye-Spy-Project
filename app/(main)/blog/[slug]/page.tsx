...
// Next.js passes it automatically because the folder is named [slug].
// If the URL is /blog/alarm-system-myths-debunked, then params.slug = "alarm-system-myths-debunked"
export default function BlogPostPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  // Find the post that matches the current URL slug
  const { slug } = React.use(params);
  const post = posts.find((p) => p.slug === slug);

  // If no matching post is found, show a 404-style message
  if (!post) {
    return (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {relatedPosts.map((related) => (
                <a
                  ...
                  className="group p-6 rounded-sm border border-white/5 hover:border-[#00c8ff]/20 bg-[#0a1628] transition-all duration-300"
                >
                  <span
                    className="text-[#00c8ff]/70 text-xs uppercase tracking-widest mb-3 block"
                    style={{ fontFamily: "'Rajdhani', sans-serif" }}
                  >
                    {related.category}
                  </span>
                  <h3
                    className="text-lg font-bold text-white group-hover:text-[#00c8ff] transition-colors duration-200 mb-2"
                    style={{ fontFamily: "'Rajdhani', sans-serif" }}
                  >
                    {related.title}
                  </h3>
